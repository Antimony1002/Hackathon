...
async function generateStudyPlanWithDeepSeek(files, targetScore, reviewDays) {
  ...
  const prompt = `作为一位专业的教学规划师，请根据以下学习资料和目标，制定一个详细、系统的复习计划：
...
请生成一个详细到每天的学习计划，要求如下：
1. 每日学习目标（3-5个具体目标）
2. 具体学习内容（按小时划分，每个时间段不少于50字的详细说明）
3. 重点难点突破（详细说明每个重点难点的学习方法和技巧）
4. 复习建议（具体可行的复习策略和方法）
5. 自我检测方法（提供具体的测试题目或检测方式）
6. 学习资源推荐（推荐相关的学习资料、视频、练习题等）
  ...
  "daily_plans": [
    {
      ...
      "schedule": [
        {"time": "9:00-10:30", "activity": "活动名称", "details": "详细的学习内容说明，不少于50字，具体描述要学习什么知识点，使用什么方法，达到什么效果"},
        {"time": "10:45-12:00", "activity": "活动名称", "details": "详细的学习内容说明，不少于50字"}
      ],
      "key_points": ["重点1", "重点2", "重点3"],
      "difficulty": "中等",
      "review_suggestions": "具体的复习建议，不少于50字",
      "self_test_methods": "具体的自我检测方法，不少于50字",
...
function generateFallbackStudyPlan(files, targetScore, reviewDays) {
  ...
  for (let i = 1; i <= reviewDays; i++) {
    dailyPlans.push({
      ...
      schedule: [
        { time: '9:00-10:30', activity: '知识点学习', details: `深入学习${subject}相关知识点，重点关注基本概念、原理和公式，通过阅读教材和参考书，理解知识点的内涵和外延，做好详细的笔记记录` },
        { time: '10:45-12:00', activity: '例题分析', details: `分析典型例题，理解解题思路和方法，总结解题规律，掌握不同类型题目的解题技巧，注意解题步骤的规范性和逻辑性` },
        { time: '14:00-15:30', activity: '习题练习', details: `完成相关习题，巩固知识点，选择不同难度的题目进行练习，从基础题到提高题，逐步提升解题能力，注意练习的针对性和有效性` },
        { time: '15:45-17:00', activity: '错题整理', details: `整理错题，分析错误原因，建立错题本，记录错误类型和改正方法，定期复习错题，避免重复犯错，提高解题准确率` },
        { time: '19:00-20:30', activity: '知识点回顾', details: `回顾当天学习内容，强化记忆，采用多种记忆方法，如思维导图、口诀、联想记忆等，加深对知识点的理解和记忆` }
      ],
      key_points: [
        `${subject}${i === 1 ? '基础知识' : i === reviewDays ? '综合应用' : '核心概念'}的深入理解`,
        `解题技巧和方法的应用`,
        `常见错误的分析和避免`,
        `知识点的系统化和网络化`
      ],
      difficulty: i === 1 ? '简单' : i === reviewDays ? '困难' : '中等',
      review_suggestions: `建议采用主动学习法，通过提问、讨论、总结等方式加深理解。合理安排复习时间，避免疲劳学习。利用多种学习资源，如教材、视频、在线课程等。定期进行自我检测，及时调整学习策略。保持良好的学习状态，注意劳逸结合，提高学习效率。`,
      self_test_methods: `通过做练习题、模拟测试、自测题等方式进行自我检测。可以采用闭卷测试检验记忆效果，开卷测试检验理解程度。对于错题要认真分析原因，找出知识薄弱点，有针对性地进行复习。建议每天学习结束后进行简短的自我测试，检验当天学习效果。`,
      learning_resources: [
        `${subject}教材及配套练习册`,
        `相关视频课程和在线教学资源`,
        `历年真题和模拟试卷`,
